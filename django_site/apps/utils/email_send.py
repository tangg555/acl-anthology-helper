...
# 导入setting中发送邮件的配置
from django_site.settings import EMAIL_FROM
...
def send_register_email(email, send_type='register'):
    ...
    if send_type == 'register':
        email_title = 'Acl-anthology-helper注册激活链接.'
        ...
        email_title = 'Acl-anthology-helper密码重置链接.'
        email_body = '请点击下面的链接重置你的密码: http://localhost:8000/reset/{0}'.format(code)
        # 使用Django内置函数完成邮件发送。四个参数：1.subject,2.message,3.from_email,4.recipient_list
        send_status = send_mail(email_title, email_body, EMAIL_FROM, [email])
        # 如果发送成功
        if send_status:
